...
ENC_KEY <- "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
...
##########################################################################################
# Example 1: Search for First 100 results of "Employment", sort by descending popularity
##########################################################################################
      # Run Search
      mySearch <- buffetSearch(query="employment",
                               start = "0",
                               rows = "100",
                               popularitySort ="desc",
                               ACC_KEY = ACC_KEY,
...
##########################################################################################
# Example 2: Search For First 5 results for Employment using DataBuffet's Default sorting
##########################################################################################
        # Run search 
        mySearch2 <- buffetSearch(query="employment",
                                 start = "0",
                                 rows = "5",
                                 ACC_KEY = ACC_KEY,
                                 ENC_KEY = ENC_KEY)            
...
##########################################################################################
# Example 3: Search For "Employment" where geo_rfa (MA Geo Code) is "IUSA_MPIT"
##########################################################################################            
      mySearch3 <- buffetSearch(query="employment|AND|geo_rfa:IUSA_MPIT",
                                ACC_KEY = ACC_KEY,
                                ENC_KEY = ENC_KEY)
